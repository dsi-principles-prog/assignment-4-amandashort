...
```{r}
first_of_month <- floor_date(today(), unit = "year") + months(0:11)
first_of_month
```
...
```{r}
stringr::words
letters <- str_view(words, "[A-z]{7,}", match = TRUE)
letters
...
str_view(words, v_alpha, match = TRUE)
```
...
words[no_vowels==TRUE]


#Verbal expression
v_no_vowels <- rx() %>%
  rx_start_of_line() %>%
  rx_anything_but(value = "aeiou") %>%
  rx_end_of_line()
v_no_vowels

str_view(words, v_no_vowels, match = TRUE)
```
...
#Verbal expression
v_rule <- rx() %>%
  rx_something_but(value = "c") %>%
  rx_find("ei") 

str_view(words, v_rule, match = TRUE)
...
telrx <- rx() %>%
  ...
  rx_end_of_line() %>%
  print() %>%
  grepl(c("317-607-9405", "317.607.9405", "(317) 607-9405")) %>%
  print()
```
